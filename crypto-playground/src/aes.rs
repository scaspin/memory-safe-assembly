...
#[bums_macros::check_mem_safe("aesv8-armx.S", input.as_ptr(), output.as_ptr(), blocks, key.as_ptr(), ivec.as_mut_ptr())]
fn aes_hw_ctr32_encrypt_blocks(
    input: &mut [u8],
    output: &mut [u8],
    blocks: usize,
...
#[bums_macros::check_mem_safe("vpaes-armv8.S", input.as_ptr(), output.as_ptr(), blocks, key.as_ptr(), ivec.as_mut_ptr())]
fn vpaes_ctr32_encrypt_blocks(
    input: &mut [u8],
    output: &mut [u8],
    blocks: usize,
...
fn vpaes_encrypt(input: &mut [u8], output: &mut [u8], key: &AesKey);

fn aes_ctr128_encrypt(key: &AesKey, ivec: &mut [u8], block_buffer: &mut [u8], in_out: &mut [u8]) {
