...
fn sha256_update_hw(ctx: &mut SHA256_CTX, msg: &[u8], len: usize) -> Result<(), ()> {
    ...
    if n != 0 {
        if len > SHA256_CBLOCK || len + n >= SHA256_CBLOCK {
            ms_memcpy(&mut ctx.data[n..], msg, SHA256_CBLOCK - n);
            sha256_block_data_order_hw(&mut ctx.h, &ctx.data);
            n = SHA256_CBLOCK - n;
            msg = &msg[n..];
            len = len - n;
            ctx.num = 0;
    ...
    n = len / SHA256_CBLOCK;
    if n > 0 {
        sha256_block_data_order_hw(&mut ctx.h, msg);
        n = n * SHA256_CBLOCK;
        msg = &msg[n..];
        len = len - n;
    }
...
fn sha256_final_hw(out: &mut [u8], ctx: &mut SHA256_CTX) -> Result<(), ()> {
    ...
    if n > (SHA256_CBLOCK - 8) {
        ms_memset(&mut ctx.data[n..], 0, SHA256_CBLOCK - n);
        n = 0;
        sha256_block_data_order_hw(&mut ctx.h, &ctx.data[0..64]);
    }
    ms_memset(&mut ctx.data[n..], 0, SHA256_CBLOCK - 8 - n);
    // Append a 64-bit length to the block and process it.
    // is big endian = true
...
mod tests {
    ...
    pub fn bench_sha256_hw_clams_assembly(b: &mut Bencher) {
        ...
        b.iter(|| {
            ...
            sha256_block_data_order_hw(&mut context, &message);
            return context;
        })
    }
    ...
    #[cfg(feature = "nightly")]
    #[bench]
    fn bench_sha256_rustcrypto_full_impl(b: &mut Bencher) {
        let mut rng = rand::thread_rng();
