...
use bums::common::*;
mod disassembler;
use disassembler::{find_and_disassemble_aarch64_function, ParsedFunction};
...
#[derive(Debug)]
struct AttributeListNoFileName {
    _separator: Option<Token![,]>,
    argument_list: Punctuated<Expr, Token![,]>,
}

impl Parse for AttributeListNoFileName {
    fn parse(input: ParseStream) -> Result<Self> {
        return Ok(Self {
            _separator: input.parse()?,
            argument_list: punctuated::Punctuated::<Expr, Token![,]>::parse_terminated(input)?,
...
fn find_binary_with_function(function_name: String) -> ParsedFunction {
    let project_root = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    match find_and_disassemble_aarch64_function(project_root, &function_name) {
        Ok(f) => f,
        Err(e) => {
            abort_call_site!(
                "Cannot find source object file for function `{}`: {}",
                function_name,
                e
            );
        }
...
pub fn check_mem_safe_from_build(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let parsed_program = find_binary_with_function(fn_name.to_string());
    ...
    engine.add_region(
        RegionType::RW,
        "memory".to_string(),
        AbstractExpression::Immediate(parsed_program.data.len() as i64 * 8),
    );
    engine.load_memory_from_vec("memory".to_string(), parsed_program.data);
    ...
    let res = engine.start_at(parsed_program.start_line);
